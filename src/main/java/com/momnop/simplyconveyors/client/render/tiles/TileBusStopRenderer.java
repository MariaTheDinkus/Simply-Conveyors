...
public class TileBusStopRenderer extends TileEntitySpecialRenderer
{
	...
	public static void renderFloatingText(String text, float x, float y, float z, int color, boolean renderBlackBox, float partialTickTime)
	{
		...
		// renderFloatingText(new String[] { ((TileEntityBusStop) te).getName()
		// }, (float) x, (float) y, (float) z, Color.WHITE.getRGB(), true,
		// partialTicks);
		...
		if(renderBlackBox)
		{
			...
			tessellator.addVertex(-stringMiddle - 1, 8 + 10 * 1 - 10, 0.0D);
			tessellator.addVertex(stringMiddle + 1, 8 + 10 * 1 - 10, 0.0D);
			tessellator.addVertex(stringMiddle + 1, -1 + 0, 0.0D);
			tessellator.draw();
	...
	{
		if(te instanceof TileBusStop)
		{
			float i = (float) x;
			float j = (float) y;
			float k = (float) z;

			renderFloatingText(((TileBusStop) te).getName(), i, j + 0.5F, k, 0xFFFFFF, true, partialTicks);
		}
	}
}
