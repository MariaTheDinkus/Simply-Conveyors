	...
	public void update()
	{
		if(!this.getWorld().isAirBlock(this.getPos().up()) && !this.getWorld().isBlockPowered(this.getPos()))
		{
			...
			if(this.getWorld().getTileEntity(this.getPos().up()) != null)
			{
				...
				entityBlock = new EntityBlock(this.getWorld(), this.getPos().getX() + 0.5, this.getPos().getY() + (1F / 16F), (this.getPos().getZ() + 0.5), this.getWorld().getBlockState(
						this.getPos().up()), tile.writeToNBT(new NBTTagCompound()));
			}
			else
			{
				entityBlock = new EntityBlock(this.getWorld(), this.getPos().getX() + 0.5, this.getPos().getY() + (1F / 16F), (this.getPos().getZ() + 0.5), this.getWorld().getBlockState(
						this.getPos().up()), new NBTTagCompound());
			}
