...
public class TileEntityDetectorBackwardsPath extends TileEntity implements ITickable
{
	...
	public void update()
	{
		...
		List entities = this.getWorld().getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(pos.getX() - 0, pos.getY() - 1, pos.getZ() - 0, pos.getX() + 1, pos.getY() + 1, pos.getZ() + 1));
								...
								if(Class.forName(string).isInstance(ent))
								{
									this.getWorld().setBlockState(
											...
											this.getWorld()
													...
													.withProperty(((BlockMovingBackwardsDetectorPath) blockType).FACING,
															this.getWorld().getBlockState(this.getPos()).getValue(((BlockMovingBackwardsDetectorPath) blockType).FACING))
													.withProperty(((BlockMovingBackwardsDetectorPath) blockType).POWERED, true));
									return;
								}
							}
						}
		...
		this.getWorld().setBlockState(
				...
				this.getWorld()
						...
						.withProperty(((BlockMovingBackwardsDetectorPath) blockType).FACING,
								this.getWorld().getBlockState(this.getPos()).getValue(((BlockMovingBackwardsDetectorPath) blockType).FACING))
						.withProperty(((BlockMovingBackwardsDetectorPath) blockType).POWERED, false));
