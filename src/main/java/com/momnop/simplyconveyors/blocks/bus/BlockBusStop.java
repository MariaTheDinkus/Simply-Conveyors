...
public class BlockBusStop extends BlockNormal implements ITileEntityProvider
{
	...
	public BlockBusStop(String unlocalizedName, Material material, float hardness, SoundType type, CreativeTabs tab)
	{
	...
	public IBlockState getStateForPlacement(World world, BlockPos pos, EnumFacing facing, float hitX, float hitY, float hitZ, int meta, EntityLivingBase placer, EnumHand hand)
	{
		if(!world.isRemote)
		{
			BusStopManager.busStopsNames.put(pos, "Bus Stop");
	...
	{
		if(worldIn.isRemote && playerIn.getHeldItem(hand) != ItemStack.EMPTY && playerIn.getHeldItem(hand).getItem() == SimplyConveyorsItems.wrench
				&& !(playerIn.getHeldItem(hand).getItem() instanceof ItemBusTicket))
		{
			playerIn.openGui(SimplyConveyors.INSTANCE, 2, worldIn, pos.getX(), pos.getY(), pos.getZ());
			return true;
		}
